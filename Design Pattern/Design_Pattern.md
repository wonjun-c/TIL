...
디자인 패턴이란?
```
객체지향의 기본만 알고있다고 해서 유연하고, 재사용이 가능하고, 관리하기 쉬운 시스템을 만드는것은 매우 어려움. 
보다 좋은 시스템을 만들기 위해 많은 사람들이 오랜시간 고민해 만들어둔것이 디자인 패턴이다.

- 소프트웨어 개발에 있어서 항상 우리 곁에 있는것이 바로 '변화'다.
  아무리 잘 만들어진 프로그램이라도 시간이 지나면 성장하고 변화가 필요하고 우리는 변화에 유연한 프로그램을 만들수 있어야한다.
